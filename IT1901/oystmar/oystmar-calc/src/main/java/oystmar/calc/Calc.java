...
public class Calc {
    ...
    public double performOperation(UnaryOperator<Double> op) throws IllegalStateException {
        if (getOperandCount() == 0) {throw new IllegalStateException("The stack is empty.");}
        var firstOp = popOperand();
        var result = op.apply(firstOp);
        pushOperand(result);
        return result;
    }
     ...
     */
    public void swap() {
        if (getOperandCount() < 2) {throw new IllegalStateException("The operand count must be bigger than two.");}
        var op2 = popOperand();
        var op1 = popOperand();
        pushOperand(op2);
        pushOperand(op1);   
